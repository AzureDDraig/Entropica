...
public class MateriaVesselControllerRenderer implements BlockEntityRenderer<MateriaVesselControllerBlockEntity, MateriaVesselControllerRenderer.VesselRenderState> {

    private static final ResourceLocation WHITE_TEXTURE = ResourceLocation.withDefaultNamespace("textures/misc/white.png");

    public MateriaVesselControllerRenderer(BlockEntityRendererProvider.Context context) {}
    ...
    @Override
    public void extractRenderState(MateriaVesselControllerBlockEntity blockEntity, VesselRenderState renderState, float partialTick, Vec3 cameraPos, @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderState.extractBase(blockEntity, renderState, crumblingOverlay);

        renderState.isFormed = blockEntity.isFormed();
        renderState.maxCapacity = blockEntity.getMaxCapacity();

        MateriaFumusStack fume = blockEntity.getStoredMateria();
