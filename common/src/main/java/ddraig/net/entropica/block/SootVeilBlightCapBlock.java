...
public class SootVeilBlightCapBlock extends BushBlock {
    public static final EnumProperty<Direction> FACING = BlockStateProperties.FACING;

    public SootVeilBlightCapBlock(Properties properties) {
        super(properties);
    ...
    protected void entityInside(BlockState state, Level level, BlockPos pos, net.minecraft.world.entity.Entity entity, net.minecraft.world.entity.InsideBlockEffectApplier effectApplier, boolean isInside) {
        ...
        if (entity instanceof LivingEntity && !level.isClientSide()) {
            triggerSootExplosion(level, pos);
        }
    }

    private void triggerSootExplosion(Level level, BlockPos pos) {
        AABB area = new AABB(pos).inflate(3.0);
        List<LivingEntity> livingList = level.getEntitiesOfClass(LivingEntity.class, area);
        for (LivingEntity target : livingList) {
            target.hurt(level.damageSources().wither(), 2.0f);
            target.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, 120, 0, false, true, true));
            target.addEffect(new MobEffectInstance(MobEffects.WITHER, 100, 0, false, true, true));
        }
    }
}
