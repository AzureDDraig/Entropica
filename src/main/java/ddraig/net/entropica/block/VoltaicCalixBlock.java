...
import ddraig.net.entropica.block.entity.VoltaicCalixBlockEntity;
...
public class VoltaicCalixBlock extends Block implements EntityBlock {

    public VoltaicCalixBlock(Properties properties) {
        super(properties);
    }
    ...
    public BlockEntity newBlockEntity(BlockPos pos, BlockState state) {
        return new VoltaicCalixBlockEntity(pos, state);
    }

    @Override
    public void setPlacedBy(Level level, BlockPos pos, BlockState state, @Nullable LivingEntity placer, ItemStack stack) {
        if (level.getBlockEntity(pos) instanceof VoltaicCalixBlockEntity cellBlockEntity) {
            CustomData customData = stack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY);
            CompoundTag tag = customData.copyTag();

            int storedMana = tag.getInt("StoredMateria5").orElse(0);
    ...
    public BlockState playerWillDestroy(Level level, BlockPos pos, BlockState state, Player player) {
        if (!level.isClientSide() && !player.isCreative()) {
            if (level.getBlockEntity(pos) instanceof VoltaicCalixBlockEntity cellBlockEntity) {
                ...
                tag.putInt("StoredMateria5", cellBlockEntity.getMateria());
