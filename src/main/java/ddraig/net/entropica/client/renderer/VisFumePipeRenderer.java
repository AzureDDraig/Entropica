...
public class VisFumePipeRenderer implements BlockEntityRenderer<VisFumePipeBlockEntity, VisFumePipeRenderer.VisFumePipeRenderState> {
    ...
    public VisFumePipeRenderer(BlockEntityRendererProvider.Context context) {}
    ...
    @Override
    public void extractRenderState(VisFumePipeBlockEntity be, VisFumePipeRenderState state, float partialTick,
                                   Vec3 cameraPos, @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderState.extractBase(be, state, crumblingOverlay);

        VisFumeStack fumeStack = be.getFumeInTank();
        ...
        state.isHoldingDetector = player != null && (
                player.getMainHandItem().is(ModItems.VIS_VALUE_DETECTOR.get()) ||
                        player.getOffhandItem().is(ModItems.VIS_VALUE_DETECTOR.get())
        );

        BlockState blockState = be.getBlockState();

        if (blockState.getBlock() instanceof VisFumeOneWayValveBlock) {
            state.isOneWayValve = true;
            state.valveFacing = blockState.getValue(VisFumeOneWayValveBlock.FACING);
        } else {
            state.isOneWayValve = false;
            state.valveFacing = null;
        }

        if (blockState.getBlock() instanceof VisFumePipeBlock) {
            state.north = blockState.getValue(VisFumePipeBlock.NORTH);
            state.south = blockState.getValue(VisFumePipeBlock.SOUTH);
            state.east  = blockState.getValue(VisFumePipeBlock.EAST);
            state.west  = blockState.getValue(VisFumePipeBlock.WEST);
            state.up    = blockState.getValue(VisFumePipeBlock.UP);
            state.down  = blockState.getValue(VisFumePipeBlock.DOWN);
        }
    }
