...
import ddraig.net.entropica.block.VisReadoutBlock;
...
public class VisReadoutRenderer implements BlockEntityRenderer<VisReadoutBlockEntity, VisReadoutRenderer.ReadoutRenderState> {
    private final Font font;

    public VisReadoutRenderer(BlockEntityRendererProvider.Context context) {
    ...
    @Override
    public void extractRenderState(VisReadoutBlockEntity blockEntity, ReadoutRenderState renderState, float partialTick, Vec3 cameraPos, @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
        BlockEntityRenderState.extractBase(blockEntity, renderState, crumblingOverlay);
        renderState.facing = blockEntity.getBlockState().getValue(VisReadoutBlock.FACING);
            ...
            for (int x = -2; x <= 2; x++) {
                for (int y = -2; y <= 2; y++) {
                    for (int z = -2; z <= 2; z++) {
                        ...
                        // 2. Check for Vis Fume Vessels or Chambers via IFumeHandler
                        else if (targetEntity instanceof IFumeHandler fumeHandler) {
                            ...
                            if (className.contains("Controller")) {
                                ...
                                VisFumeStack fume = fumeHandler.getFumeInTank();
