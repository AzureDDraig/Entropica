...
public class CreativeVisFumeGeneratorRenderer implements BlockEntityRenderer<CreativeVisFumeGeneratorBlockEntity, CreativeVisFumeGeneratorRenderer.GeneratorRenderState> {

    private final Font font;

    public CreativeVisFumeGeneratorRenderer(BlockEntityRendererProvider.Context context) {
    ...
    @Override
    public void extractRenderState(CreativeVisFumeGeneratorBlockEntity be, GeneratorRenderState state, float partialTick, Vec3 cameraPos, @Nullable ModelFeatureRenderer.CrumblingOverlay crumblingOverlay) {
    ...
    public void submit(GeneratorRenderState state, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraRenderState) {
        ...
        for (Direction dir : Direction.Plane.HORIZONTAL) {
            ...
            // 1. Push it to the very edge of the specific face (+0.505 prevents Z-fighting)
            poseStack.translate(
            ...
            // 3. Flip Y so text isn't upside down, and scale it to fit nicely on the block
        ...
        // CRITICAL FIX: Flush the buffer to force the text to draw to the screen!
